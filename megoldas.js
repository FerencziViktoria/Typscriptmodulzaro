...
function LeetKod(vizsgaltSzoveg) {
    let tomb = ["i", "I", "o", "O", "a", "A", "e", "E"];
    var modositottSzo = "";
    for (let i = 0; i < vizsgaltSzoveg.length; i++) {
        let cserelniKell = false;
...
var snookerAdatok = ["52;Akani Sunny;Thaiföld;118500",
...
function ObjektumFeltolto(feltoltendoElem) {
    ...
    for (let i = 0; i < feltoltendoElem.length; i++) {
        let elemDarabok = feltoltendoElem[i].split(" ");
        let objektum = {
...
var snookerinfo = ObjektumFeltolto(snookerAdatok);
...
function LegtobbNyeremeny(adatok) {
    let legtobbNyeremenyIndex = 0;
    for (let i = 0; i < adatok.length; i++) {
