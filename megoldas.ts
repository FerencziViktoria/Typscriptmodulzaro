...
function HosegriadoSzint(nap1: number, nap2: number, nap3: number): number {
    if (nap1 || nap2 || nap3 >= 25) {
        return ("1");

    }
    else if (nap1 && nap2 && nap3 >= 25) {
...
function OszthatoSzamok(oszto: number, vizsgaltTomb: number[]): number {
    let oszthatokmennyisege: number = 0;
...
//4. feladat
function Erettsegi(pontok: number[]): [összpontszám: number, jegy: number] {
    let összpontszám: number = 0;
    for (let i: number = 0; i < pontok.length; i++) {
        összpontszám += pontok[i]; {
            let jegy: number = 0;
            if (összpontszám <= 39) {
                jegy == 1;
            }
            else if (összpontszám <= 59) {
                jegy == 2;
            }
            else if (összpontszám <= 79) {
                jegy == 3;
            }
            else if (összpontszám <= 119) {
                jegy == 4;
            }
            else if (összpontszám <= 150) {
                jegy == 5;
            }
            else (){
                return "Hibás adat";
...
var snookerAdatok:string[] = ["52;Akani Sunny;Thaiföld;118500",
...
interface snookerAdatok{
  ...
  var snookerinfo:snookerAdatok[]=ObjektumFeltolto(snookerAdatok);
  //Feltöltés ellenőrzése
  function AdatKiir(adatok:snookerAdatok[]):void{
    for(let i:number=0;i<adatok.length;i++){
        console.log(adatok[i].Helyezes+"-"+adatok[i].Nev+"-"+adatok[i].Orszag+"-"+adatok[i].Nyeremeny);
    }
  }
  function LegtobbNyeremeny(adatok:snookerAdatok[]):number{
    let legtobbNyeremenyIndex:number=0;
    for(let i:number=0;i<adatok.length;i++){
        if(adatok[i].Nyeremeny>adatok[legtobbNyeremenyIndex].Nyeremeny){
            legtobbNyeremenyIndex=i;
        }
    }
    return legtobbNyeremenyIndex;
}
